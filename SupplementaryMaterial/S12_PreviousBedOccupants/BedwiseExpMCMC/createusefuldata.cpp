...
void previouspatientstatus(vector<int> & previouspatarray, vector<int> & previousCol, vector <int> & acquisition, vector <int> & admissionstate){
  int pat=0, previous=0;
  std::vector<int>::iterator it;
  for(pat=0;pat<previousCol.size();pat++){
    it = std::find(previouspatarray.begin(), previouspatarray.end(), pat);
    if(it!=previouspatarray.end()){
      previous = std::distance(previouspatarray.begin(), it);
      cout << "Pat =" << pat << ", previous=" << previous << endl;
      if(acquisition.at(previous)==1 || admissionstate.at(previous)==1){
        previousCol.at(pat)=1;
      }else previousCol.at(pat)=0;
    }else{
      previousCol.at(pat)=0;
    }
  }
}
void updatepreviouscol(int patient, vector<int> & previouspatarray, vector<int> & previousCol, vector <int> & acquisition, vector <int> & admissionstate){
  int next = previouspatarray.at(patient);
  // If patient is found then update previousCol
  if(next!=-1){
    previousCol.at(next)=acquisition.at(patient) + admissionstate.at(patient);
    if(acquisition.at(patient)==1 && admissionstate.at(patient)==1){
      cout << "Error: Patient" << patient << "has acquisition and importation." <<endl;
    }
  }
  // If patient is not found (i.e. not a previous patient of someone), then nothing to do
}
...
//   int pat=0, previous=0;
//   for(pat=0;pat<previousCol.size();pat++){
//     previous = previouspatarray.at(pat);
//     if(previous==-1) previousCol.at(pat)=0;
//     else{
//       if(acquisition.at(previous)==1 || admissionstate.at(previous)==1){
//         previousCol.at(pat)=1;
//       }else previousCol.at(pat)=0;
...
// void updatepreviouscol(int patient, vector<int> & previouspatarray, vector<int> & previousCol, vector <int> & acquisition, vector <int> & admissionstate){
//   std::vector<int>::iterator it;
//   it = std::find(previouspatarray.begin(), previouspatarray.end(), patient);
//   // If patient is found then update previousCol
//   if(it!=previouspatarray.end()){
//     // Determine index, index = patient
//     int pat = std::distance(previouspatarray.begin(), it);
//     previousCol.at(pat) = acquisition.at(patient) + admissionstate.at(patient);
//     if(acquisition.at(patient)==1 && admissionstate.at(patient)==1){
//       cout << "Error: Patient" << patient << "has acquisition and importation." <<endl;
//     }
//   }
